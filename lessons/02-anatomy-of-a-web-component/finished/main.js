...
class NMAButton extends HTMLElement {
  constructor() {
    ...
    // Create the 'a' tag that will be rendered to the
    // Shadow DOM
    const buttonElement = document.createElement("a");
    // Build the 'buttonElement' base off attributes
    // passed to the custom element
    buttonElement.href = this.getAttribute("href");
    buttonElement.innerHTML = this.innerHTML;

    // Check for variants to the button and update
    // the classList
    ...
    const styleTag = this.getStyles();

    // Add the styles and button element to the
    // Shadow DOM
    this.shadowRoot.append(styleTag, buttonElement);
  }

  /* Returns a 'style' tag with the 'styles'
...
// Define the custom element so that it can be used
// in the HTML
